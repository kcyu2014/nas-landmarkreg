...
from . import net_plotter as net_plotter
from . import plot_2D as plot_2D
from . import plot_1D as plot_1D
from . import model_loader as model_loader
    ...
    if os.path.exists(surf_file):
        if args.no_resume:
            ...
            if (args.y and 'ycoordinates' in f.keys()) or 'xcoordinates' in f.keys():
                ...
                return
            # Enter error state, delete the file.
            f.close()
            os.remove(surf_file)
...
def crunch(surf_file, net, w, s, d, dataloader, loss_key, acc_key, comm, rank, args):
    ...
    eval_loss_fn = evaluation.eval_supernet_loss if args.eval_loss_type == 'supernet' else evaluation.eval_loss
    ...
    for count, ind in enumerate(inds):
        ...
        loss, acc = eval_loss_fn(net, criterion, dataloader, args.cuda)
