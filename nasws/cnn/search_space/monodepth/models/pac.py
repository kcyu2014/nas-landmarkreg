...
def nd2col(
    ...
    if transposed:
        ...
        input_nd = F.pad(
            input_nd,
            (pad[1], pad[1] + output_padding[1], pad[0], pad[0] + output_padding[0]),
        )
        stride = _pair(1)
        padding = _pair(0)
...
class GaussKernel2dFn(Function):
    ...
    def forward(ctx, input, kernel_size, stride, padding, dilation, channel_wise):
        ...
        out_h = (
            in_h + 2 * ctx.padding[0] - ctx.dilation[0] * (ctx.kernel_size[0] - 1) - 1
        ) // ctx.stride[0] + 1
        out_w = (
            in_w + 2 * ctx.padding[1] - ctx.dilation[1] * (ctx.kernel_size[1] - 1) - 1
        ) // ctx.stride[1] + 1
...
class PacConv2dFn(Function):
    ...
    def forward(
        ...
        ctx.save_for_backward(
            input if (ctx.needs_input_grad[1] or ctx.needs_input_grad[2]) else None,
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0] or ctx.needs_input_grad[1]:
            if ctx.shared_filters:
                grad_in_mul_k = grad_output.view(
                    bs, out_ch, 1, 1, out_sz[0], out_sz[1]
        ...
        if ctx.needs_input_grad[1] or ctx.needs_input_grad[2]:
            in_cols = F.unfold(
                input, ctx.kernel_size, ctx.dilation, ctx.padding, ctx.stride
            )
        ...
        if ctx.needs_input_grad[2]:
            ...
            if ctx.shared_filters:
                ...
                grad_weight = grad_weight.view(
                    1, 1, ctx.kernel_size[0], ctx.kernel_size[1]
                ).contiguous()
            else:
...
class PacConvTranspose2dFn(Function):
    ...
    def forward(
        ...
        x = F.pad(
            ...
            (
                pad[1],
                pad[1] + ctx.output_padding[1],
                pad[0],
                pad[0] + ctx.output_padding[0],
            ),
        )
...
class PacPool2dFn(Function):
    ...
    def forward(ctx, input, kernel, kernel_size, stride=1, padding=0, dilation=1):
        ...
        ctx.stride = _pair(stride)
        ctx.save_for_backward(
            input if ctx.needs_input_grad[1] else None,
...
def packernel2d(
    ...
    if transposed:
        in_sz = tuple(
            ...
            for (o, k, s, p, op, d) in zip(
                input.shape[-2:], kernel_size, stride, padding, output_padding, dilation
            )
        )
    else:
        in_sz = input.shape[-2:]
    ...
    if mask is not None or normalize_kernel:
        ...
        if mask is not None:
            ...
            if not normalize_kernel:
                norm = mask.sum(dim=2, keepdim=True).sum(
                    dim=3, keepdim=True
    ...
    if native_impl:
        ...
        if kernel_type == "gaussian":
            ...
            epsilon = 1e-4
            x = inv_alpha.view(1, -1, 1, 1, 1) + torch.pow(
                x + epsilon, 0.5 * inv_lambda.view(1, -1, 1, 1, 1)
            )
        else:
            raise ValueError()
    ...
    if norm is not None:
        empty_mask = norm == 0
        output = output / (
            norm + torch.tensor(empty_mask, dtype=input.dtype, device=input.device)
        )
...
def pacconv_transpose2d(
    ...
    if native_impl:
        ...
        x = F.pad(
            x, (pad[1], pad[1] + output_padding[1], pad[0], pad[0] + output_padding[0])
        )
        output = pacconv2d(
            x,
            kernel,
        ...
        output = PacConvTranspose2dFn.apply(
            input,
            kernel,
            weight,
            bias,
...
def pacpool2d(
    input, kernel, kernel_size, stride=1, padding=0, dilation=1, native_impl=False
):
    kernel_size = _pair(kernel_size)
    stride = _pair(stride)
    padding = _pair(padding)
    ...
    if native_impl:
        ...
        out_h = (
            in_h + 2 * padding[0] - dilation[0] * (kernel_size[0] - 1) - 1
        ) // stride[0] + 1
        out_w = (
            in_w + 2 * padding[1] - dilation[1] * (kernel_size[1] - 1) - 1
        ) // stride[1] + 1
...
class _PacConvNd(nn.Module):
    def __init__(
        ...
        if (
            smooth_kernel_type.find("_") >= 0
            and int(smooth_kernel_type[smooth_kernel_type.rfind("_") + 1 :]) % 2 != 1
        ):
            raise ValueError("smooth_kernel_type only accept kernels of odd widths")
        if shared_filters:
            assert (
                in_channels == out_channels
            ), "when specifying shared_filters, number of channels should not change"
        if any(
            [p > d * (k - 1) / 2 for (p, d, k) in zip(padding, dilation, kernel_size)]
        ):
        ...
        if not pool_only:
            if self.filler in {"pool", "crf_pool"}:
                ...
                if self.filler == "crf_pool":
                    self.weight[
                        (0, 0) + tuple(k // 2 for k in kernel_size)
                    ] = 0  # Eq.5, DenseCRF
            elif shared_filters:
        ...
        if kernel_type.startswith("inv_"):
            ...
            raise ValueError(
                "kernel_type set to invalid value ({})".format(kernel_type)
            )
        if smooth_kernel_type.startswith("full_"):
            smooth_kernel_size = int(smooth_kernel_type.split("_")[-1])
            self.smooth_kernel = Parameter(
                torch.Tensor(1, 1, *repeat(smooth_kernel_size, len(kernel_size)))
            )
            ...
            raise ValueError(
                "smooth_kernel_type set to invalid value ({})".format(
                    smooth_kernel_type
                )
            )
    ...
    def reset_parameters(self):
        if not (self.pool_only or self.filler in {"pool", "crf_pool"}):
            if self.filler == "uniform":
                ...
                assert all(
                    k >= ek for k, ek in zip(self.kernel_size, effective_kernel_size)
                )
                w = 1.0
                for i, (p, s, k) in enumerate(zip(pad, self.stride, self.kernel_size)):
                    ...
                    if self.normalize_kernel:
                        w = w * np.array(
                            tuple(((k - j - 1) // s) + (j // s) + 1.0 for j in range(k))
                        ).reshape((-1,) + (1,) * d)
                ...
                gauss = (
                    np_gaussian_2d(self.kernel_size[0])
                    * self.kernel_size[0]
                    * self.kernel_size[0]
                )
