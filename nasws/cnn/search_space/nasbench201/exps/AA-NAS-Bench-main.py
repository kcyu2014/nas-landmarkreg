...
def generate_meta_info(save_dir, max_node, divide=40):
  aa_nasbench101_ss = get_search_spaces('cell', 'aa-nas')
  archs = CellStructure.gen_all(aa_nasbench101_ss, max_node, False)
  print ('There are {:} archs vs {:}.'.format(len(archs), len(aa_nasbench101_ss) ** ((max_node-1)*max_node/2)))
