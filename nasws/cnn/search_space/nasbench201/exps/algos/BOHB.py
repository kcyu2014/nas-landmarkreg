...
class MyWorker(Worker):
  ...
  def __init__(self, *args, sleep_interval=0, convert_func=None, nasbench101=None, **kwargs):
    super().__init__(*args, **kwargs)
    self.sleep_interval = sleep_interval
    self.convert_func   = convert_func
    self.nasbench101      = nasbench101
    self.test_time      = 0

  def compute(self, config, budget, **kwargs):
    structure = self.convert_func( config )
    reward    = train_and_eval(structure, self.nasbench101, None)
    self.test_time += 1
    return ({
...
def main(xargs):
  ...
  for i in range(num_workers):
    w = MyWorker(nameserver=ns_host, nameserver_port=ns_port, convert_func=config2structure, nasbench101=nasbench101, run_id=hb_run_id, id=i)
