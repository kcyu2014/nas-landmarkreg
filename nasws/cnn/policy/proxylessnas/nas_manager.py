...
class ArchSearchRunManager(CNNSearchPolicy):
    ...
    def __init__(self, path, super_net, run_config: RunConfig, arch_search_config: ArchSearchConfig, args):
        ...
        super(CNNSearchPolicy, self).__init__(args)
        ...
        self.warmup = True
        self.warmup_epoch = 0
        self.model = self.run_manager.net
    ...
    def load_model(self, model_fname=None):
        ...
        if self.warmup and 'warmup_epoch' in checkpoint:
            ...
            if isinstance(self.warmup_epoch, tuple):
                self.warmup_epoch = self.warmup_epoch[0]

        print(f"Loading from checkpoint finished. Start from "
              f"{self.run_manager.start_epoch} with warmup epoch {self.warmup} / {self.warmup_epoch} ")
    ...
    def log_architecture(self, epoch):
    ...
    def save_network_config(self, epoch=None, prefix='learned_net', mode='chosen'):
        """ save network config as normal net, can be used to trained from scratch directly... """
        # convert to normal network according to architecture parameters
        normal_net = self.net.cpu().convert_to_normal_net(mode)
        print('Total training params: %.2fM' % (count_parameters(normal_net) / 1e6))
        network_config_path = os.path.join(
            self.run_manager.path,
            '{}-{}'.format(prefix, epoch) if epoch else prefix)
        os.makedirs(network_config_path, exist_ok=True)
        json.dump(normal_net.config, open(os.path.join(network_config_path, 'net.config'), 'w'), indent=4)
        json.dump(
            self.run_manager.run_config.config,
            open(os.path.join(network_config_path, 'run.config'), 'w'), indent=4,
        )
        torch.save(
            {'state_dict': normal_net.state_dict(), 'dataset': self.run_manager.run_config.dataset},
            os.path.join(network_config_path, 'init')
        )
