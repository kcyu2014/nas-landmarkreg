...
parser.add_argument('--dataset_dir', type=str, default='data', help="dataset location")
...
parser.add_argument('--resume', type=str2bool, default=True, help='resume the training or not.')
...
parser.add_argument('--n_worker', type=int, default=2, help='nb worker')
...
parser.add_argument('--epochs_lr', type=int, default=-1, help='num of training epochs')
...
parser.add_argument('--valid_portion', type=float, default=0.0, help='by default it is 1 - train_portion, specify otherwise')
parser.add_argument('--arch_learning_rate', type=float, default=3e-4, help='learning rate for arch encoding')
parser.add_argument('--arch_weight_decay', type=float, default=1e-3, help='weight decay for arch encoding')
parser.add_argument('--label_smooth', type=float, default=0, help='weight decay for arch encoding')

# Args for for supernet...
parser.add_argument('--supernet_train_method', type=str, default='darts', help='Training method for supernet model')
parser.add_argument('--supernet_cell_type', type=str, default='op_on_node')
parser.add_argument('--supernet_warmup_epoch', type=int, default=15, help='supernet warmup epoch before any sampler. disabled for SPOS series sampler')
parser.add_argument('--supernet_warmup_method', type=str, default='oneshot', choices=['darts', 'oneshot', 'softoneshot'])
parser.add_argument('--supernet_eval_method', type=str, default='oneshot', choices=['darts', 'oneshot', 'softoneshot'])
...
# Settings for MixedVertex Op in nasbench101.operations 
...
parser.add_argument('--channel_dropout_method', type=str, default='fixed_chunck',
                    help='Channel dropout configs. Used in '
                         'nasbench101.operations.ChannelDropout')
...
parser.add_argument('--policy_args', default=None, help='Placeholder for policy args')
...
parser.add_argument('--landmark_sample_method', type=str, default='fixed', help="Sampling method for landmarks")
...
parser.add_argument('--landmark_forward_mode', type=str, default='oneshot', help="Use weighted loss in this one.", choices=['oneshot', 'softoneshot'])
parser.add_argument('--landmark_sample_distance', type=int, default=0, help="sampling distance")


parser.add_argument('--softoneshot_alpha', default=0.05, type=float, help='Soft-one shot alphas')
...
parser.add_argument('--nasbench101_fix_last_channel_sum', type=int, default=2, 
                    help='Indicates the last layer mutation id let us consider the current implement now.')
parser.add_argument('--nasbench102_dataset', type=str, default='cifar10', help='NB201 dataset indicator')
parser.add_argument('--nasbench102_use_valid_only', type=str2bool, default=False, help='set to True, only enable the valid ones.')
parser.add_argument('--nasbench102_use_isomorphic', type=str2bool, default=False, help='set to True, use only isomorphic architecture.')
parser.add_argument('--nasbench102_use_isomorphic_consider_zeros', type=str2bool, default=False, help='consider only zeros')
parser.add_argument('--darts_search_shuffle', type=str, default='', help='DARTS search space')
...
parser.add_argument("--path_sample_method", type=str, default='default', help='Path Sampling Tricks. in | default | random_nas')
...
# Loss landscape configs here
parser.add_argument("--plot_loss_landscape", action='store_true', help='Plot the loss landscape')
parser.add_argument("--plot_loss_landscape_noresume", action='store_true', help='no resume')
parser.add_argument("--plot_loss_landscape_always_same_arch", default=-1, type=int, help='set to flag the single arch, -1 indicates none, use full')
parser.add_argument("--plot_loss_landscape_step", default=51, type=int, help='steps between -1 1')
