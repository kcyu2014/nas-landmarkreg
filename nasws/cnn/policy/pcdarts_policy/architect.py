...
class Architect(object):
    ...
    def _compute_unrolled_model(self, input, target, eta, network_optimizer):
        loss = self.module_forward_fn(self.model, input, target)
    ...
    def _backward_step(self, input_valid, target_valid):
        loss = self.module_forward_fn(self.model, input_valid, target_valid)
        loss.backward()
    ...
    def _backward_step_unrolled(self, input_train, target_train, input_valid, target_valid, eta, network_optimizer):
        ...
        unrolled_loss = self.module_forward_fn(unrolled_model, input_valid, target_valid)
        ...
        return unrolled_loss.data.item()
...
class PCDartsArchitect(Architect):

    def __init__(self, model, args, module_forward_fn):
        """[summary]
        ...
        args : [type]
            [description]
        module_forward_fn : method
            take (model, inputs, outputs) and return the loss to compute DARTS update.
        """
        super().__init__(model, args, module_forward_fn)
        self.model_new = deepcopy(model)

    def _construct_model_from_theta(self, theta):
        # logging.debug('Create a new model, should change to reset_parameters.')
