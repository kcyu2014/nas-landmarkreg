...
def _summarize_shared_train(
    curr_step, total_loss, raw_total_loss, acc=0, acc_5=0, lr=0.0, epoch_steps=1, writer=None, prefix=''
    ):
...
def _profile_timer(timers: dict):
    logstr = '|'.join( [f'{k} {v.avg:.2f}' for k, v in timers.items()])
    logging.info(logstr)
...
def darts_model_validation(valid_queue, model, criterion, args, verbose=True, early_stop=0):
    objs = nasws.cnn.utils.AverageMeter()
    top1 = nasws.cnn.utils.AverageMeter()
    top5 = nasws.cnn.utils.AverageMeter()
    model.eval()
    with torch.no_grad():
        for step, (input, target) in enumerate(valid_queue):
            if args.debug:
                if step > 10:
                    logging.debug("Break after 10 batch")
                    break
            input = input.cuda()
            target = target.cuda()
            ...
            if step >= early_stop > 0:
                logging.debug('break due to early stop...')
                break
    
    return top1.avg, objs.avg


def darts_train_model_apex_dali(train_queue, valid_queue, model, criterion, optimizer, lr, args, architect, sampler=None):
    objs = nasws.cnn.utils.AverageMeter()
    top1 = nasws.cnn.utils.AverageMeter()
    top5 = nasws.cnn.utils.AverageMeter()
    # IPython.embed()
...
def darts_model_validation_apex_dali(valid_queue, model, criterion, args, verbose=True):
    objs = nasws.cnn.utils.AverageMeter()
    top1 = nasws.cnn.utils.AverageMeter()
    top5 = nasws.cnn.utils.AverageMeter()
    model.eval()
