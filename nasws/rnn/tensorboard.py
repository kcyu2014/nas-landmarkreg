        ...
        for idx, img in enumerate(value):
            ...
            if type(img) == str:
                if not os.path.exists(img):
                    # logger.debug(f"Image {img} not exists, possible due to there is no graphviz module.")
                    return
...
class TensorboardRank(TensorBoard):
    """
    Create for generate the ranking plot during runtime.
