...
import utils
from nasws.rnn.enas_policy import enas_utils as utils
...
class RNN(shared_base.SharedModel):
    """Shared RNN model."""
    def __init__(self, args, corpus, logger=None):
        shared_base.SharedModel.__init__(self)
        self.logger = logger or utils.get_logger('enas_rnn')
        self.args = args
        self.corpus = corpus
    ...
    def forward(self,  # pylint:disable=arguments-differ
        ...
        if clipped_num > 0:
            self.logger.debug(f'clipped {clipped_num} hidden states in one forward '
                              f'pass. '
    ...
    def cell(self, x, h_prev, dag):
        ...
        :param x: input at timestep.
        :param h_prev: (previous hidden state)
        :param dag: Directed acyclic graph, which defines the sampled architecture.
                (SHOULD be a path).
        :return:
        """
        c = {}
        ...
        while True:
            ...
            node_id = q.popleft()
            nodes = dag[node_id] # TODO What is dag actually?
        ...
        # (brendan): Instead of averaging loose ends, perhaps there should
        ...
        if self.args.shared_average_all:
            leaf_nodes = [h[node_id] for node_id in range(0, self.args.num_blocks)]
        else:
    ...
    def get_num_cell_parameters(self, dag):
        ...
        num += shared_base.size(self.w_xc)
        num += shared_base.size(self.w_xh)
        ...
        while True:
            ...
            for next_node in nodes:
                ...
                num += shared_base.size(w_h)
                num += shared_base.size(w_c)

                q.append(next_id)

        self.logger.debug(f'# of cell parameters: '
